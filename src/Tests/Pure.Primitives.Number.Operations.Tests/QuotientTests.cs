...
public sealed record QuotientTests
{
    ...
    {
        Assert.Throws<NotSupportedException>(() => new Quotient<float>(new Float(10)).GetHashCode());
    }

    [Fact]
    public void ThrowsExceptionOnToString()
    {
        Assert.Throws<NotSupportedException>(() => new Quotient<float>(new Float(10)).ToString());
    }
}