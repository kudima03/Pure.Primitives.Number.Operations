...
public sealed record LessThanConditionTests
{
    ...
    {
        Assert.Throws<InvalidOperationException>(() => new LessThanCondition<float>(new Float(10)).GetHashCode());
    }

    [Fact]
    public void ThrowsExceptionOnToString()
    {
        Assert.Throws<InvalidOperationException>(() => new LessThanCondition<float>(new Float(10)).ToString());
    }
}