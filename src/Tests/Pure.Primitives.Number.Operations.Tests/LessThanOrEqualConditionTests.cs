...
public sealed record LessThanOrEqualConditionTests
{
    ...
    {
        Assert.Throws<InvalidOperationException>(() => new LessThanOrEqualCondition<float>(new Float(10)).GetHashCode());
    }

    [Fact]
    public void ThrowsExceptionOnToString()
    {
        Assert.Throws<InvalidOperationException>(() => new LessThanOrEqualCondition<float>(new Float(10)).ToString());
    }
}